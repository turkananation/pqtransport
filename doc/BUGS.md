...
Do not vendor ChaCha or ECDH.

## Won't fix in 0.1 (honest limits)

| ID | Summary | Why |
|---|---|---|
| LIM-01 | OpenSSL / BoringSSL handshake | Needs a recorded transcript. Hellos, raw-pk, HRR, IANA `0x1302`/`0x1303` are on the wire. |
...
| BLK-02 | TLS | Local HMAC Expand only. | `hkdfExtract`/`hkdfExpand` call pqforge SHA-256 RFC 5869; A.1 pin kept. SHA-384 schedule is OPEN-02 **Fixed**. |
...
| OPEN-01 | TLS | Compact private hellos. | RFC 8446 ClientHello/ServerHello + extensions. Compact body retired. Cipher IANA `0x1302`/`0x1303`. `test/tls/rfc8446_hello_test.dart`. |
